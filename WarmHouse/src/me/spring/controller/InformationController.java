...
public class InformationController {
    ...
    public String houseInfoEdit(String code, Model model) {
		...
		// 无感埋点：记录“浏览”行为（1代表浏览）。埋点失败不影响页面正常展示。
		try {
			User user = (User) session.getAttribute("user");
			if (user != null && houseinfo != null) {
				Integer houseId = houseinfo.getId();
				userBehaviorService.recordBehavior(user.getId(), houseId, 1);
			}
		} catch (Exception e) {
			// 埋点异常吞掉，避免影响正常页面流程
		}
    ...
    public String addHouseInfoSubmit(HouseInformation houseinfo, Model model) {
		Result result = new Result();
		// 租赁系统：房源发布不允许手动填写编号，后端自动生成唯一编号
		houseinfo.setCode("RENT-" + System.currentTimeMillis());
		System.out.println(houseinfo);
