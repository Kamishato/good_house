...
public class UserImp implements UserService {
	...
	public int addHeadimg(User user, CommonsMultipartFile file, HttpServletRequest request) {
		// 1. 防御：如果未选择文件或文件为空，直接跳过
		if (file == null || file.isEmpty()) {
			return 0;
		}

		// 2. 清理：先删除该用户可能存在的旧头像关联，防止出现多条记录导致查询报错
		userDAO.deleteUserimg(user.getUsername());
		...
		// 4. 保存物理文件并获取图片信息
		Map<String, Object> result = RequestEntity.saveSingleFile(request, file);
		if (result == null || result.get("imageinfo") == null) {
			return 0; // 文件保存失败兜底
		}
