...
<body>
    ...
    <c:if test="${pageInfo != null && pageInfo.total > 0}">
        <div class="simple-pagination" style="text-align: center; margin-bottom: 20px; font-size: 14px; color: #666; width: 100%;">
            <span style="margin-right: 15px;">共 ${pageInfo.total > 50 ? 50 : pageInfo.total} 条浏览记录</span>
            <a href="${pageContext.request.contextPath}/user/history?pageNum=1" style="margin: 0 5px; color: #007bff; text-decoration: none;">首页</a>
            <c:if test="${pageInfo.pageNum > 1}">
                <a href="${pageContext.request.contextPath}/user/history?pageNum=${pageInfo.pageNum - 1}" style="margin: 0 5px; color: #007bff; text-decoration: none;">上一页</a>
            </c:if>
            <span style="margin: 0 10px;"> 第 ${pageInfo.pageNum} / ${pageInfo.pages > 5 ? 5 : pageInfo.pages} 页 </span>
            <c:if test="${pageInfo.pageNum < (pageInfo.pages > 5 ? 5 : pageInfo.pages)}">
                <a href="${pageContext.request.contextPath}/user/history?pageNum=${pageInfo.pageNum + 1}" style="margin: 0 5px; color: #007bff; text-decoration: none;">下一页</a>
            </c:if>
            <a href="${pageContext.request.contextPath}/user/history?pageNum=${pageInfo.pages > 5 ? 5 : pageInfo.pages}" style="margin: 0 5px; color: #007bff; text-decoration: none;">尾页</a>
        </div>
    </c:if>
